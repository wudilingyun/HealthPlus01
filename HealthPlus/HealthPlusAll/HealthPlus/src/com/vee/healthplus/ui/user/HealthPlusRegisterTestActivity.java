...
import com.vee.healthplus.util.user.RegisterTask.RegisterCallBack;
...
public class HealthPlusRegisterTestActivity extends BaseFragmentActivity
	...
	public HealthPlusRegisterTestActivity() {
	...
	public void onClick(View view) {
			...
			if (checkMobileNumber(userName_et.getText().toString())) {
				...
				if (userPwd_et.getText().toString()
					...
					if (!m.matches() && !m1.matches() && m2.matches()) {
						if (s >= 6 && s <= 12) {
							new RegisterTaskTest(this, userName_et.getText()
		...
		case R.id.health_plus_register_read:
			Intent intent = new Intent(HealthPlusRegisterTestActivity.this,
	...
	class RegisterTaskTest extends AsyncTask<Void, Void, Void> {
		...
		public RegisterTaskTest(Activity activity, String userName,
				String password, String verifycode, String nick,
				RegisterCallBack callBack,
				SignInTask.SignInCallBack signInCallBack) {
		...
		public RegisterTaskTest(Activity activity, String userName,
				String password, String userNick, RegisterCallBack callBack) {
			this.activity = activity;
			this.username = userName;
			this.password = password;
			this.callBack = callBack;
		...
		protected Void doInBackground(Void... params) {
			try {
				...
				System.out.print("registerResponse="
			...
			if (registerResponse != null) {
				...
				if (registerResponse.getReturncode() == 8) {
					...
					if (signInCallBack != null) {
						HP_User.setOnLineUserId(activity, Integer
								.valueOf(String.valueOf(registerResponse
										.getMemberid())));
						new SignInTask(activity, username, password,
								signInCallBack).execute();
					}
				}
			}
		}
